...
public class FSDirectory implements Closeable {
  static final Log LOG = LogFactory.getLog(FSDirectory.class);
  ...
  BlockInfo addBlock(String path, INodesInPath inodesInPath, Block block,
    ...
    try {
      ...
      for (int i=arrayInodes.length-1; i>=0; i--) {
        if (arrayInodes[i] != null) {
          ...
          for (XAttr xattr : xattrs) {
            if (xattr.getName().equals(FairIOFlowManager.nameWeight)) {     // entra en aquest
      ...
      fileINode.addBlock(blockInfo);        // TODO TODO crear block
   ...
   * @deprecated Use {@link #renameTo(String, String, long, Rename...)}
   */
  @Deprecated
   ...
   * @deprecated See {@link #unprotectedRenameTo(String, String, long, Rename...)}
   */
  @Deprecated
