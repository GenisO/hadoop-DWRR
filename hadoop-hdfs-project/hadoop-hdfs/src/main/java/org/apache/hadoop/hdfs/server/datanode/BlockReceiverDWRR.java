 ...
 */
class BlockReceiverDWRR implements Closeable {
  ...
  // TODO TODO atributs propis
  private boolean responderClosed = false;
  private LinkedList<WriteRequestDWRR> toBeWritten;

  BlockReceiverDWRR(final ExtendedBlock block, final DataInputStream in,
    ...
    try {
      this.toBeWritten = new LinkedList<WriteRequestDWRR>();
      this.block = block;
      this.in = in;
      this.inAddr = inAddr;
      this.myAddr = myAddr;
      ...
      DataNode.LOG.warn("IOException in BlockReceiverDWRR constructor. Cause is ",
        cause);
  ...
  private int receivePacket() throws IOException {
    ...
    if (mirrorOut != null && !mirrorError) {
      try {
        ...
        long duration = Time.monotonicNow() - begin;
        if (duration > datanodeSlowLogThresholdMs) {
          LOG.warn("Slow BlockReceiverDWRR write packet to mirror took " + duration
...
//          }
  ...
  private void finalizeReceivePacket(WriteRequestDWRR req) throws IOException {
    long offsetInBlock = req.getOffsetInBlock();
    int len = req.getLen();
    boolean lastPacketInBlock = req.getLastPacketInBlock();
    boolean syncBlock = req.getSyncBlock();
    ...
    try {
      // TODO TODO Write data to disk
      long begin = Time.monotonicNow();
      out.write(req.getArray(), req.getStartByteToDisk(), req.getNumBytesToDisk());
      long duration = Time.monotonicNow() - begin;
      if (duration > datanodeSlowLogThresholdMs) {
        LOG.warn("Slow BlockReceiverDWRR write data to disk cost:" + duration
          + "ms (threshold=" + datanodeSlowLogThresholdMs + "ms)");
      }
      ...
      if (partialCrc != null) {
        ...
        partialCrc.update(req.getArray(), req.getStartByteToDisk(), req.getNumBytesToDisk());
        ...
        lastChunkChecksum = Arrays.copyOfRange(
          req.getChecksumArray(),
          req.getChecksumBuf().arrayOffset() + req.getChecksumBuf().position() + req.getChecksumLen() - checksumSize,
          req.getChecksumBuf().arrayOffset() + req.getChecksumBuf().position() + req.getChecksumLen());
        checksumOut.write(req.getChecksumArray(),
          req.getChecksumBuf().arrayOffset() + req.getChecksumBuf().position(),
          req.getChecksumLen());
      }
      ...
      replicaInfo.setLastChecksumAndDataLen(
        last.getDataLength(), lastChunkChecksum
      );
  ...
  int receiveBlock(
    ...
    try {
      ...
      // TODO TODO receivePacket de tots els paquets sense fer escriptures i deixarlos a memoria (partir el metode receivePacket)
      int len = -1;
      boolean nextRecivePacket = true;
      while (nextRecivePacket) {		/* Receive until the last packet */
        len = receivePacket();
        if (len>= 0) {
          size += len;
        }
        else nextRecivePacket = false;
      }
      // TODO TODO fer les escriptures dels paquets (part del metode receivePacket) i continuar per aqui
  ...
  void finalizeReceiveBlock() throws IOException {
    try {
      ...
      if (isClient) {
        ...
        if (ClientTraceLog.isInfoEnabled() && isClient) {
          ...
          ClientTraceLog.info(String.format(DN_CLIENTTRACE_FORMAT, inAddr,
            ...
            dnR.getDatanodeUuid(), block, endTime));
        } else {
      ...
      if (!responderClosed) { // Data transfer was not complete.
        LOG.info("CAMAMILLA "+this+" finally finalizeReceiveBlock responderClosed "+responderClosed);      // TODO TODO log
        if (responder != null) {
          ...
          if (datanode.isRestarting() && isClient && !isTransfer) {
            ...
            LOG.info("CAMAMILLA "+this+" finally finalizeReceiveBlock send OOB Ack");      // TODO TODO log
            try {
  ...
  class PacketResponder implements Runnable, Closeable {
    ...
    @Override
    public void close() {
      LOG.info("CAMAMILLA thread close responder "+ackQueue.size()+" abans del synchronized isRunning "+isRunning());
      synchronized (ackQueue) {
        while (isRunning() && ackQueue.size() != 0) {
          LOG.info("CAMAMILLA thread close responder "+ackQueue.size());
          try {
            ackQueue.wait();
          } catch (InterruptedException e) {
            running = false;
        ...
        running = false;
        LOG.info("CAMAMILLA thread close responder notifyAll ");
        ackQueue.notifyAll();
      }

      synchronized (this) {
        LOG.info("CAMAMILLA thread close responder altre synchronized");
        running = false;
        notifyAll();
      }
    }
      ...
      while (isRunning() && !lastPacketInBlock) {
        ...
        try {
          ...
          */
          ...
          LOG.warn("IOException in BlockReceiverDWRR.run(): ", e);
          if (running) {
            datanode.checkDiskError();
            LOG.info(myString, e);
            running = false;
     ...
     * Finalize the block and close the block file
     *
     * @param startTime time when BlockReceiverDWRR started receiving the block
     */
    private void finalizeBlock(long startTime) throws IOException {
      BlockReceiverDWRR.this.close();
      final long endTime = ClientTraceLog.isInfoEnabled() ? System.nanoTime()
        : 0;
      block.setNumBytes(replicaInfo.getNumBytes());
      datanode.data.finalizeBlock(block);
