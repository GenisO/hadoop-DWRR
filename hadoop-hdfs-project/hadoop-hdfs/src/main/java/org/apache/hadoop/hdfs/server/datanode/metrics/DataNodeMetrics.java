...
public class DataNodeMetrics {
  public static final Log LOG = LogFactory.getLog(DataNodeMetrics.class);

  // TODO TODO metrics
  @Metric MutableCounterLong bytesWritten;
  @Metric MutableCounterLong bytesRead;
  @Metric MutableCounterLong totalBytes;
  ...
  public DataNodeMetrics(String name, String sessionId, int[] intervals,
    ...
    //processedRequests = new MutableRates(new MetricsRegistry("datanode"));
  ...
  public void incrProcessedRequest(String classId, long bytes, float weight) {
    ...
    registry.add(name, bytes);
  }

  public void setQueuedRequests(String classId, int queuedRequests) {
    String desc = "classId_metrics";
    String name = "classId_"+classId+"_";
    if (registry.get(name) == null) {
      registry.newStatShare(name, desc, "Ops", "Bytes", true, totalBytes);
    ...
    MutableStatShare reg = ((MutableStatShare) registry.get(name));
    reg.setQueuedRequests(queuedRequests);
  }
}
