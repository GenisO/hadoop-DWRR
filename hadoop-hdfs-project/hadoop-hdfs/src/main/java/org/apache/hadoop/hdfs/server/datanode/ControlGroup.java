...
public abstract class ControlGroup {
    ...
    /**
     * An interface to the blkio sub-system of Linux control groups. Currently, this
     * implementation controls only IO BW shares of each group. To use a IO cgroup,
     * create a new group and set the IO share allocated to the members of the
     * group.
     */
    public static class BlkIOControlGroup extends ControlGroup {
        /*
         * blkio.weight         # contains an integer value that specifies a relative
         *                      share of IO time available to the tasks in a cgroup
         * blkio.time           # time spend by the cgroup doing IO on device
         *
         */
        public static final String IO_WEIGHT = "blkio.weight";
        public static final String IO_TIME = "blkio.time";

        public static final long IO_WEIGHT_DEFAULT = 500;

        public static final String SUBSYS_BLKIO = "blkio";

        public static boolean isAvailable() {
            return isSubsystemAvailable(SUBSYS_BLKIO);
        }

        public BlkIOControlGroup() {
            super("/sys/fs/cgroup/blkio");
        }

        public BlkIOControlGroup(String path) {
            super(path);
        }

        public BlkIOControlGroup getSubGroup(String name) {
            return new BlkIOControlGroup(getSubDirectory(name));
        }

        public BlkIOControlGroup createSubGroup(String name) {
            return new BlkIOControlGroup(createSubDirectory(name));
        }

        public long getIOShares() {
            return getLongParameter(IO_WEIGHT);
        }

        public void setIOShares(long value) {
            setLongParameter(IO_WEIGHT, value);
        }

        public boolean canControl() {
            return super.canControl() && canWrite(IO_WEIGHT) && canMkdir("");
        }
    }
    /*
    ...
    public void addProcToGroup(String pgrp) {
        setStringParameter(CG_PROCS, pgrp);
    }

    public void addProcsToGroup(String[] pgrpList) {
        for(String pgrp: pgrpList) {
            addProcToGroup(pgrp);
        }
    }

    public void addTaskToGroup(String pgrp) {
        setStringParameter(CG_TASKS, pgrp);
    }

    public void addTasksToGroup(String[] pgrpList) {
        for(String pgrp: pgrpList) {
            addTaskToGroup(pgrp);
        }
    }
